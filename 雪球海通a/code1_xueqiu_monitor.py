...
# 雪球组合
portfolio_code = "ZH2133642"  # 神威，木叶村
portfolio_name =  "(神威，木叶村)"
# portfolio_code = "ZH2314973"  # 养股-超级成长股，随风一起舞啊
# portfolio_code = "ZH2180545"    # 从0-1，大发
# portfolio_code = "ZH2895043"  # 风雨同心-古戈尔

# portfolio_code = "ZH2954711"  # 阿凯测试01，测试用
...
def spider(port):
    ...
    while True:
        ...
        # 上午收盘后到下午开盘前，休眠
        if '125900' > (datetime.datetime.now().strftime('%H%M%S')) > '113000':
            time.sleep(10)
            return
        ...
        try:
            ...
            if latest_update_time > record_data["update_time"]:
                ...
                if holding_df.empty:
                    print('调仓价格为0，不更新时间戳')
                    continue
                holding_df['stock_symbol'] = holding_df['stock_symbol'].str.slice(2) + '.' + holding_df['stock_symbol'].str.slice(0, 2)
                holding_df.loc[holding_df['stock_symbol'].str.startswith('1'), 'segment_name'] = '可转债'
